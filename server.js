...
app.post('/api/run', (_req,res) => {
  ...
  state.runCount += 1;
  res.json({ok:true, message:'Scrape pipeline completed', source:state.source, runCount:state.runCount, listings:state.listings.map(analyze)});
});
app.post('/api/scrape', (req,res) => {
  ...
  state.lastRun = new Date().toISOString();
  state.runCount += 1;
  res.json({ok:true, mode:'brightdata-ready', message:'Scrape request accepted. Connect your Bright Data Scraper Studio dataset/API here.', url, listings:state.listings.map(analyze)});
});
